...
export async function updateUser({
  ...
  try {
    ...
    // Update global state with the new user information to get the fresh data
  ...
  try {
    ...
    const newPet = await databases.createDocument(
      ...
      ID.unique(),
      {
        ...
        type: form.type,
        name: form.name,
        weight: parseInt(form.weight, 10), // Ensure weight is an integer
...
export async function deletePet(petId) {
  try {
    const deletedPet = await databases.deleteDocument(
      appwriteConfig.databaseId,
      appwriteConfig.petsCollectionId,
      petId
    );
